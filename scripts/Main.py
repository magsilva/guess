...
def showTitle(title,time):
	vf.showTitle(title,time)
...
def setTitleColor(aColor):
	vf.setTitleColor(aColor)
...
def weightedKKLayout(*d):
	if len(d) == 0:
		g.weightedKKLayout()
	elif len(d) == 1:
		g.weightedKKLayout(d[0])
	else:
		raise ValueError("Incorrect number of arguments")
...
def setQuality(requestedQuality):
	v.setQuality(requestedQuality)
