...
public class SGraph {
     ...
     */
    public SGraph(int a_nodecount, SEdge a_edge[]) {
	nodecount = a_nodecount;
	edge = new SEdge[a_edge.length];
	...
	for (x=0; x<a_edge.length; x++) {
	    ...
	    edge[x]=new SEdge(edgeFrom,edgeTo);
     ...
     */
    public SGraph(Reader r) throws IOException {
	...
	for (linebuf = br.readLine(); !(linebuf.equals(".")); linebuf = br.readLine()) {
	    ...
	    edges.add(new SEdge(edgeFrom,edgeTo));
	    edgesFrom[edgeFrom].add(new Integer(edgeTo));
	    edgesTo[edgeTo].add(new Integer(edgeFrom));
	}
	edge = new SEdge[edges.size()];
	edges.toArray(edge);
	acyclic = false;
	reduced = false;
    }
     ...
     */
    public SGraph getGraphWithoutOneOrTwoCycles() {
	...
	SEdge newEdge[] = new SEdge[newEdges.size()];
	newEdges.toArray(newEdge);
	return new SGraph(nodecount,newEdge);
    }
     ...
     */
    public SGraph getGraphWithoutMultipleEdges() {
	LinkedList newEdges = new LinkedList();
	int edgeFrom;
	for (edgeFrom=0; edgeFrom<nodecount; edgeFrom++) {
		...
		if (!seenEdgeTo.contains(edgeTo)) {
		    newEdges.add(new SEdge(edgeFrom,edgeTo.intValue()));
     ...
     */
    public SGraph[] partition(int partitionIndex[], int nodeRenumber[]) {
	...
	for (x=0; x<edge.length; x++) {
	    ...
	    partitionEdges[partitionIndex[e.getFrom()]].add(
							    new SEdge(nodeRenumber[e.getFrom()],
								     nodeRenumber[e.getTo()]));
	}
	SGraph retval[] = new SGraph[numberOfPartitions];
	for (x=0; x<numberOfPartitions; x++) {
	    SEdge pe[] = new SEdge[partitionEdges[x].size()];
	    partitionEdges[x].toArray(pe);
	    retval[x] = new SGraph(partitionNodecount[x],pe);
	}
	return retval;
    }
	...
	SGraph simplifiedGraph = getGraphWithoutMultipleEdges().getGraphWithoutOneOrTwoCycles();
	if (nodecount == 2) {
     ...
     */
    public SGraph getGraphWithoutCycles(int cycleEliminationPriority[]) {
	...
	for (x=0; x<edge.length; x++) {
		...
		edgeFrom = edgeTo;
		edgeTo = temp;
		newEdges.add(new SEdge(edgeFrom,edgeTo));
	    } else {
		newEdges.add(edge[x]);
	    }
	}
	SEdge newEdge[] = new SEdge[newEdges.size()];
	newEdges.toArray(newEdge);
	SGraph dag = new SGraph(nodecount,newEdge);
	dag.acyclic = true;
	return dag;
    }
     ...
     */
    public SGraph getReducedGraph(int topologicalOrder[]) {
	...
	for (nodeIndex = topologicalOrder.length - 1; nodeIndex >= 0; nodeIndex--) {
		...
		if (descendants[nodeId] == null) {
		    ...
		    newEdges.add(new SEdge(nodeId,daughterId));
		} else {
	...
	SEdge newEdge[] = new SEdge[newEdges.size()];
	newEdges.toArray(newEdge);
	SGraph reducedGraph = new SGraph(nodecount,newEdge);
     ...
     * graph.
     */
    public SGraph getReducedGraph() {
	int cycleEliminationPriority[] = getCycleEliminationVertexPriority();
	SGraph dag = getGraphWithoutCycles(cycleEliminationPriority);
	...
	try {
	    SGraph graph = new SGraph(new InputStreamReader(System.in));
		...
		*/
		...
		SGraph red = component[x].getReducedGraph();
