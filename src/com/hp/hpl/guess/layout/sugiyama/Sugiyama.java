...
package com.hp.hpl.guess.layout.sugiyama;
...
class HierarchyFlowLayoutOrderNode implements Comparable {
	...
	public HierarchyFlowLayoutOrderNode(Node a_nodeView, 
	...
	public int compareTo(Object o) {
		HierarchyFlowLayoutOrderNode y = 
		    (HierarchyFlowLayoutOrderNode)o;
...
public class Sugiyama {
	...
	public void actionPerformed(ActionEvent event) {
		...
		TaskManager.executeTask(new Sugiyama(), config);
	}
	 */

    public Sugiyama(Graph g) {
	layout(g);
    }
	...
	try {
	    ...
	    selectedNodes.addAll(s);
	    final int numSelectedNodes = selectedNodes.size();
	    final int numNodes = numSelectedNodes;
	    final int numLayoutNodes = numNodes;

	    HashMap<Node,Integer> nToID = new HashMap<Node,Integer>();
	    Node[] nodeView = new Node[numNodes];
	    for (int i = 0 ; i < numNodes ; i++) {
		nodeView[i] = (Node)selectedNodes.get(i);
		nToID.put(nodeView[i],new Integer(i));
	    }
	    /* create edge list from edges between selected nodes */
	    LinkedList edges = new LinkedList();
	    Iterator iter = g.getEdges().iterator();
	    // load up the edges
	    while (iter.hasNext()) {
		Edge e = (Edge)iter.next();
		edges.add(new SEdge(nToID.get(e.getNode1()).intValue(),
				    nToID.get(e.getNode2()).intValue()));
	    }

	    /* find horizontal and vertical coordinates of each node */
	    SEdge edge[] = new SEdge[edges.size()];
	    edges.toArray(edge);
	    SGraph graph = new SGraph(numLayoutNodes,edge);

	    int cI[] = graph.componentIndex();
	    int x;

	    int renumber[] = new int[cI.length];
	    SGraph component[] = graph.partition(cI,renumber);
		...
		SGraph reduced = component[x].getReducedGraph();
		layer[x] = reduced.getVertexLayers();
		horizontalPosition[x] = reduced.getHorizontalPosition(layer[x]);
	    }

	    /* order nodeviews by layout order */
	    HierarchyFlowLayoutOrderNode flowLayoutOrder[] = 
		new HierarchyFlowLayoutOrderNode[numLayoutNodes];
	    for (x=0; x<numLayoutNodes; x++) {
		flowLayoutOrder[x] = 
		    new HierarchyFlowLayoutOrderNode(nodeView[x], cI[x], 
		...
		Node currentView = node.nodeView;
		if (lastComponent == -1) {
		...
		if (lastComponent != currentComponent) {
		    /* new component */
		    layerCenter[lastLayer] = 
			(startComponentX + cleanLayerX - 
		    ...
		    int backIndex;
		    for (backIndex=nodeIndex - 1;  
			 backIndex >= 0; backIndex--) {
			HierarchyFlowLayoutOrderNode backNode = 
			    flowLayoutOrder[backIndex];
			int backComponent = backNode.componentNumber;
			if (backComponent != lastComponent) {
			    break;
			}
			int backLayer = backNode.layer;
			backNode.setXPos(backNode.getXPos() + 
					 maxLayerCenter - 
					 layerCenter[backLayer]);
		    }
		    ...
		    /* new layer */
		    layerCenter[lastLayer] = (startComponentX + 
					      cleanLayerX - 
					      nodeHorizontalSpacing) / 2;
		    if (layerCenter[lastLayer] > maxLayerCenter) 
			maxLayerCenter = layerCenter[lastLayer];
		...
		int currentBottom = 
		    startLayerY + (int)(currentView.getHeight());
